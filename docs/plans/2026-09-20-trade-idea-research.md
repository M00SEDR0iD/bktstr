...
> Implement and verify one task at a time. The implementation is complete locally; see the verification report for evidence and limits.
...
**Status:** RF-A through RF-K are implemented locally. Original Tasks 0-2 are complete.
Original Tasks 3-7 retain their IDs and remain pending. See the
[verification report](../RESEARCH_FOUNDATION_VERIFICATION.md) and
[research guide](../IDEA_RESEARCH_GUIDE.md) for delivered contracts and limits.
This document retains the implementation checklist as the acceptance record.
...
The named research modules are implemented. Inspect existing seams before each task; do not
...
- [x] Write tests rejecting unknown fields, mutable nested state, missing disproof
  criteria, duplicate/conflicting revision identity, unpinned ancestry, and policy
  fields in a study modifier.
- [x] Run `python -m pytest tests/test_research_ideas.py -q`; confirm contract failures.
- [x] Implement the records, including an idea with no policy and explicit links
  from a policy to study results and a promotion rationale.
- [x] Add a toy VWAP-reclaim idea. Its event is a same-session close crossing above
...
- [x] Verify key-order stability and semantic-versus-wording revisions; commit.
...
- [x] Test that role, numeric/boolean/ordinal value type, units, and evidence trust
  remain independent. Reject a label in a predictor dependency graph, cycles,
  unknown formulas, and missing availability rules.
- [x] Run `python -m pytest tests/test_research_components.py -q`; confirm failures.
- [x] Define versioned event detector, context, reference, and label contracts.
  Include units, inputs, lookback, availability, missing behavior, and profile.
- [x] Reuse existing VWAP, RSI, volume ratio, and numerical regime measurements.
  Add only the fixture's missing measurements with explicit formulas and timing.
- [x] Implement initial label definitions for fixed-horizon returns and favorable/
...
- [x] Run component and existing measurement tests; commit.
...
- [x] Test changed upstream data, corrupted artifacts, adjustment/unit mismatch,
  duplicates, out-of-order bars, missing minutes, warm-up, holiday/early-close
  schedules, and an offline miss.
- [x] Run `python -m pytest tests/test_dataset_snapshots.py -q`; confirm failures.
- [x] Persist raw/derived hashes, source and formula/build identity, adjustment
...
- [x] Audit expected bars against the supplied schedule. A controlled study blocks
  missing required coverage; exploratory exclusions are explicit and counted.
- [x] Publish artifacts atomically before marking ready. Replay missing/corrupt
  inputs fails without network fallback.
- [x] Verify current backtests retain numerical equivalence and baseline session
...
- [x] Write fixtures containing several same-session VWAP reclaims, an event while
...
- [x] Run `python -m pytest tests/test_event_research.py -q`; confirm failures.
- [x] Materialize event IDs and per-value availability. Keep missing context rows
  with reasons; apply only the predeclared detector and sampling rule.
- [x] Generate labels separately. Require the exact scheduled horizon price, censor
  an incomplete horizon, and exclude outcomes crossing a scored split. Do not
  reinterpret 15 minutes as 15 available rows.
- [x] Add a future-perturbation test: changing bars strictly after a cutoff leaves
  events and predictors through the cutoff identical, while affected labels change.
- [x] Verify on manually calculated fixture rows, including positive/negative
...
- [x] Test conflicting revision writes, input validation, idempotent retries,
  preflight failures, artifact publication interruption, restart, and handle closure.
- [x] Run `python -m pytest tests/test_configured_research.py -q`; confirm failures.
- [x] Add catalog/attempt links through additive migrations. Persist exact inputs
  before execution and dispatch D through the existing worker lifecycle.
- [x] Store causal/label artifacts separately. Attachments and consumed inputs have
  different provenance fields. Persist admitted blocked/failed/empty attempts.
- [x] Verify store reopening, legacy experiment retrieval, and Windows file release.
...
- [x] Test overlapping splits/label spans, warm-up scored by mistake, changed
  universes, two concurrent reservations for one slot, repeated HTTP keys, renamed
  ideas, and overlapping final-data exposure.
- [x] Run `python -m pytest tests/test_research_protocol.py -q`; confirm failures.
- [x] Freeze candidates, universe, splits, metrics, sample minima, analysis choices,
...
- [x] Test a known-effect fixture, null fixture, missing groups, small sample,
  correlated same-session events, changed event samples, and different horizons.
- [x] Run `python -m pytest tests/test_event_studies.py -q`; confirm failures.
- [x] Report event/session counts, mean/median/quantiles, favorable/adverse excursions,
  missing/censored counts, and predeclared instrument/period stability.
- [x] Fit quantile groups/scaling on development data only and pin parameters before
  later splits. Label context-group contrasts as associations.
- [x] Implement deterministic contiguous-session block resampling. Freeze block
...
- [x] Estimate differences directly using shared resamples. Fixed-event variants
...
- [x] Attach search counts and exposure status. Unavailable uncertainty remains
...
- [x] Test cycles, conflicting modifiers, missing pinned references, unknown
...
- [x] Run `python -m pytest tests/test_idea_resolution.py tests/test_configured_research.py -q`;
  confirm missing policy-path failures.
- [x] Resolve one parent and explicit typed changes. Require study evidence links,
...
- [x] Map only supported policy fields to today's strict strategy document and
  run through the shared runtime with C inputs. Preserve early entry validation.
- [x] Persist manifests/results and link signals to event IDs when definitions
  match. Disclose unmatched/unavailable mappings rather than claiming equivalence.
- [x] Verify next-bar eligibility, unchanged baseline behavior, and two applications
...
- [x] Test base plus two variants on two symbols, zero events/trades, cancellation,
  partial completion, mismatched inputs, and crashes before/after publication.
- [x] Run `python -m pytest tests/test_research_campaigns.py -q`; confirm failures.
- [x] Reuse pinned inputs within comparison cells. Reconcile completed/pending
  children and recover interrupted inline work without duplicate admitted attempts.
- [x] Report per-symbol effects and predeclared aggregation with dispersion.
  Keep study associations, policy PnL, and cost/risk sensitivities in separate panels.
- [x] Verify budgets and exposure survive retries and restart; commit.
...
- [x] Test lineage, pagination/filters, auth/validation, progress/cancellation,
  blocked/scenario labels, and exposure recording on raw-label/report exports.
- [x] Run `python -m pytest tests/test_idea_reports.py tests/test_api_ideas.py -q`;
  confirm failures.
- [x] Generate JSON and Markdown idea cards containing thesis, study components,
...
- [x] Add history filters for idea, kind, variant, instrument, campaign, status,
  and date. Show current metric definitions and execution limits.
- [x] Update delivered capabilities and OpenAPI only; commit.
...
- [x] Demonstrate one VWAP-reclaim thesis, base plus two study variants, two stocks,
  context/label reports, an inconclusive assessment, and a documented policy choice.
- [x] Run the policy base plus two variants through a controlled frozen-data campaign.
  Interrupt/resume, edit a revision, retrieve all attempts, and replay an older run.
- [x] Include failures for leakage, invalid pairing, missing/corrupt data, unavailable
  macro consumption, budget races, and reused final data claimed as untouched.
- [x] Configure an explicit persistent research root. Back up and restore the SQLite
  database together with every referenced data, schedule, evidence, and result artifact.
- [x] Run focused tests, `python -m pytest tests -q`,
...
- [x] Record delivered capabilities and known execution/metric limitations; commit.
...
- Canonical JSON records provide deep immutability and typed boundary validation.
- The numerical catalog reuses registered intraday measurements; unsupported daily/macro inputs fail.
- Both paths initialize indicators in scored sessions; broader prior-session warm-up is not implicit.
- Fixture data lives in the demonstration generator and test helpers rather than a separate event JSON file.
- Markdown idea cards and per-experiment test reports are generated automatically and can be regenerated.
- Snapshot acquisition accepts explicit validated frames and schedules; existing providers remain separate acquisition adapters.
- The original execution model remains unchanged; event-to-trade mapping is explicitly unavailable.
- Native Windows test/commit commands and a local execution ledger were used in the existing dedicated feature branch.
